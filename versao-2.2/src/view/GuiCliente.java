...
public class GuiCliente extends JPanel {
	private JCheckBox chFidelidade;
	...
	public void inicializarComponentes() {
		...
		table = new JTable();
		chFidelidade = new JCheckBox("Fidelidade");
		
		chFidelidade.setBounds(304, 123, 92, 20);
		add(chFidelidade);
	...
	private void limparCampos() {
		...
		tfLogin.setText("");
		tfSenha.setText("");
		chFidelidade.setSelected(false); // Desmarca o checkbox
	...
	public void definirEventos() {
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// Carrega a fidelidade na caixinha (coluna 10 da tabela)
				Object fid = table.getValueAt(table.getSelectedRow(), 10);
				if (fid instanceof Boolean) {
					chFidelidade.setSelected((Boolean) fid);
				}
			}
		});
		...
		btSalvar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// Se o campo ID estiver vazio, definimos como 0 para indicar um novo registro
				if (tfIdUsuario.getText().trim().isEmpty()) {
					c.setIdUsuario(0);
				} else {
					c.setIdUsuario(Integer.parseInt(tfIdUsuario.getText().trim()));
				}
				...
				c.setFidelidade(chFidelidade.isSelected()); // Captura o valor do Checkbox
		...
		btExcluir.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				// Evita erro se clicar em excluir com o campo vazio
				if(tfIdUsuario.getText().isEmpty()){
					JOptionPane.showMessageDialog(btExcluir, "Selecione ou busque um cliente para excluir.");
					return;
				}

				int r = JOptionPane.showConfirmDialog(btExcluir, "Tem certeza que deseja excluir este cliente?");
				if(r == 0) {
					// Define o ID no objeto cliente atual
					c.setIdUsuario(Integer.parseInt(tfIdUsuario.getText()));
					
					// Chama o DAO passando o objeto (descomentado e corrigido)
					JOptionPane.showMessageDialog(btExcluir, dao.excluir(c));
		...
		btBuscar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				c = dao.localizar(Integer.parseInt(tfIdUsuario.getText().trim()));
				if(c!=null) {
					...
					chFidelidade.setSelected(c.getFidelidade()); // Atualiza o checkbox na busca
				}
				else {
